...
*   **Error Handling**: Classify every failure per the Halt Protocol: unrecoverable / out-of-scope / user interrupt / retry-budget exhausted → **HALT** with Acknowledge -> Analyze -> Propose -> Ask Permission; routine recoverable failures within approved scope are self-repaired without halting. Owner: `~/.claude/skills/remy-plan/halt_protocol.md`.
