...
Log Change generates a structured changelog recording modifications, Q&A decisions, and systemic impact. The changelog serves as an audit source for `/auditor` and a context preservation mechanism for `/rewind`.

## When to Use

- After each code modification, to record what changed and why
- Before running `/auditor`, which requires a changelog as input
