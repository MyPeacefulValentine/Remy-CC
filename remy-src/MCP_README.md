...
| Requirement | Notes |
| :--- | :--- |
| `remy-daemon` binary | Deployed to `~/.remy-cc/bin/` by `install.py` |
...
On startup, `init_freshness()` (`remy-daemon/src/mcp/freshness.rs`) checks whether the index is current:
...
Check:
1. `~/.remy-cc/bin/remy-daemon --version` runs and reports the deployed version
...
### Source Files (Rust host, `remy-daemon/src/mcp/`)
...
1. Implement `query_xxx_impl(...)` in the domain module owning the concern under `remy-daemon/src/mcp/` (create a new module for a new domain, e.g. `deps.rs`).
