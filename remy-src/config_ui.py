...
PARAM_REGISTRY = [
    ...
    {"key": "LOGIC_DENSITY_FULL_MAX", "group": "injection", "type": "int", "default": "200",
     "min": 0, "max": 50000,
     "desc_en": "File count threshold for full density injection (all symbols with summaries)",
     "desc_zh": "全密度注入（含摘要）的文件数阈值"},
    {"key": "LOGIC_DENSITY_COMPACT_MAX", "group": "injection", "type": "int", "default": "500",
     "min": 0, "max": 50000,
     "desc_en": "File count threshold for compact injection (symbols without summaries). Must be >= LOGIC_DENSITY_FULL_MAX",
     "desc_zh": "紧凑注入（无摘要）的文件数阈值。须 >= LOGIC_DENSITY_FULL_MAX"},
    {"key": "LOGIC_DENSITY_CORE_MAX", "group": "injection", "type": "int", "default": "2000",
     "min": 0, "max": 100000,
     "desc_en": "File count threshold for core-only injection. Must be >= LOGIC_DENSITY_COMPACT_MAX. Above this: top_n mode",
     "desc_zh": "仅 Core 层注入的文件数阈值。须 >= LOGIC_DENSITY_COMPACT_MAX。超出此值使用 top_n 模式"},
