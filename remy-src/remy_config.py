...
_FIELDS = (
    ...
    _field("REMY_LLM_TLS_INSECURE", None, "bool", "false", "llm_api", "Disable TLS certificate verification for the LLM endpoint (insecure)", "禁用LLM端点的TLS证书校验（不安全）", label_en="Disable TLS Verification", label_zh="禁用TLS证书校验", restart_scope="next_index", advanced=True, project_allowed=False),
    ...
    _field("REMY_SYMBOL_SUMMARY_MODE", None, "enum", "auto", "summary", "Symbol-layer summary generation mode", "符号层摘要生成模式", label_en="Symbol Summary Mode", label_zh="符号摘要模式", restart_scope="next_index", options=("auto", "ask", "never")),
    _field("REMY_SYMBOL_AUTO_SIZE_GUARD", None, "int", "300", "summary", "Pending-symbol guard for automatic symbol summaries", "自动符号摘要的待处理符号数限制", label_en="Auto Symbol Guard", label_zh="自动符号摘要上限", unit_en="symbols", unit_zh="符号", restart_scope="next_index", minimum=10, maximum=100000),
    ...
    _field("REMY_PERMISSION_GATE", None, "bool", "true", "system", "Auto-approve Edit/Write/Read and Grep/Glob permission prompts for project .claude/ system artifacts", "自动放行项目级.claude/系统工件的Edit/Write/Read及Grep/Glob权限弹窗", label_en="Permission Gate", label_zh="权限闸门", restart_scope="immediate", advanced=False),
    _field("REMY_DAEMON_AUTOSTART", None, "bool", "true", "system", "Idempotently start the resident daemon at session start", "会话启动时幂等拉起常驻daemon", label_en="Daemon Autostart", label_zh="daemon自启动", restart_scope="next_session", advanced=False),
    ...
    _field("REMY_UPDATE_TIMEOUT", None, "int", "30", "system", "remy-cc update network timeout in seconds (probe and download each)", "remy-cc update 网络超时秒数（探测与下载分别应用）", label_en="Update Timeout", label_zh="更新超时", unit_en="seconds", unit_zh="秒", restart_scope="immediate", minimum=5, maximum=600),
    ...
    _field("REMY_CONFIG_UI_IDLE_TIMEOUT", None, "int", "0", "system", "Managed config UI idle exit timeout in seconds without page heartbeat (0 = never)", "托管配置UI空闲退出超时秒数（无页面心跳；0=永不退出）", label_en="Config UI Idle Timeout", label_zh="配置UI空闲超时", unit_en="seconds", unit_zh="秒", restart_scope="immediate", minimum=0, maximum=86400),
