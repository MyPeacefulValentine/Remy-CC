...
| 权限闸门 | Edit/Write/Read 权限决策时 | 自动放行针对项目级 `.claude/` 系统工件（temp 目录、history、生成的树与索引文件）、`~/.claude/projects/<slug>/memory/` 下的项目记忆文件及系统临时目录内文件的 Edit/Write/Read 弹窗；settings 类文件照常弹窗；可用 `REMY_PERMISSION_GATE` 关闭 |
