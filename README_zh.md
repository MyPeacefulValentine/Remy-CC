...
| 权限闸门 | Edit/Write 权限决策时 | 自动放行项目级 `.claude/` 系统工件写入（temp 目录、history、生成的树与索引文件）及 `~/.claude/projects/<slug>/memory/` 下的项目记忆文件；settings 类文件照常弹窗；可用 `REMY_PERMISSION_GATE` 关闭 |
