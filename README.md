...
| Permission Gate | On Edit/Write permission prompts | Auto-approves writes to project-level `.claude/` system artifacts (temp dirs, history, generated trees and indexes), per-project auto-memory files under `~/.claude/projects/<slug>/memory/`, and files inside the system temporary directory; settings files always prompt; disable via `REMY_PERMISSION_GATE` |
