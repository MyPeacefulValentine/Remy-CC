...
python -m pytest tests/test_c_fnptr_dispatch.py -q -p no:cacheprovider
```
...
All committed tests use synthetic source, temporary directories, and temporary SQLite databases. They do not require an LLM API key or network access. The public TEE repository scan is not part of this CI baseline. P0.4 will fix its repository revision, exclusion rules, and expected measurements.
